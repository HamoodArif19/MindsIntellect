...
const Pending = () => {
  ...
  }, [token,role,username,pendingQuestions]);
  ...
  return (
    ...
    >
      ...
      <div className="row">
        <div className="col-lg-12">
          <div className="card qna">
            <div className="card-header px-3 py-1 mb-3" style={{ borderBottom: "1px solid white" }}>
              <h1 className="m-3">Here are your {role==='ROLE_LEARNER'?'Questions pending for Answer.':'Anwers pending for Rating.'}</h1>
            </div>
            <div className="card-body">
              {pendingQuestions.map((qora, index) => (
                <div key={index} className="card mb-4" style={{ minHeight: "20vh" }} onClick={() => handleShow((role==='ROLE_LEARNER'?qora.questionId:qora.answerId))}>
                  <div className="row align-items-center text-center" style={{ minHeight: "20vh" }}>
                    ...
                    <div className="col-lg-7" style={{ borderLeft: "1px solid white", borderRight: "1px solid white" }}>
      ...
      {quesId !== '' && role === 'ROLE_LEARNER'? <QuestionModal showModal={showModal} handleClose={handleClose} type='pending' quesId={quesId} token={token} /> : ''}
      {quesId !== '' && role === 'ROLE_MENTOR'? <AnswerModal showModal={showModal} handleClose={handleClose} type='pending' ansId={quesId} token={token} /> : ''}
    </div>
  );
};
